...
<section class="mx-auto max-w-7xl px-4 py-12">
	<div class="mb-6 flex items-center justify-between">
		<h2 class="text-2xl font-bold text-gray-900">{t('home.featured')}</h2>
	...
	{:else}
		<p class="py-12 text-center text-gray-500">{t('home.noProducts')}</p>
	{/if}
</section>

{#if data.categories.length}
	<section class="mx-auto max-w-7xl px-4 pb-12">
	<h2 class="mb-6 text-2xl font-bold text-gray-900">{t('home.shopByCategory')}</h2>
			<div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
				{#each data.categories as cat (cat.id)}
					...
					>
						...
						<p class="mt-1 text-sm text-gray-500">{cat.productCount} {t('home.products')}</p>
					</a>
			{/each}
		</div>
	</section>
