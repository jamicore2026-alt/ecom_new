...
export const foodOrdersModule = new Elysia({ prefix: '/api' })
  ...
  .get('/food-orders', async ({ query, auth, merchantContext }) =>
    FoodOrdersService.list(auth.db, auth.merchant.id, query, merchantContext), {
    query: foodOrderQuery,
    detail: { summary: 'List food orders with filters' }
  })
  .get('/food-orders/:id', async ({ params, auth, merchantContext }) =>
    FoodOrdersService.get(auth.db, auth.merchant.id, params.id, merchantContext), {
  ...
  .post('/food-orders', async ({ body, auth, request, merchantContext }) => {
    const result = await FoodOrdersService.create(auth.db, auth.merchant.id, body, merchantContext)
  ...
  .put('/food-orders/:id', async ({ params, body, auth, request, merchantContext }) => {
    const result = await FoodOrdersService.update(auth.db, auth.merchant.id, params.id, body, merchantContext)
    await auditFromRequest(auth, request, { action: 'food_order.update', entityType: 'order', entityId: params.id })
    return result
  ...
  .post('/food-orders/:id/status', async ({ params, body, auth, request, merchantContext }) => {
    const result = await FoodOrdersService.transition(auth.db, auth.merchant.id, params.id, body.status, merchantContext)
    await auditFromRequest(auth, request, { action: 'food_order.status', entityType: 'order', entityId: params.id, metadata: { status: body.status } })
    return result
  }, { params: foodOrderParams, body: foodOrderStatusBody })
  .post('/food-orders/:id/cancel', async ({ params, auth, request, merchantContext }) => {
    const result = await FoodOrdersService.cancel(auth.db, auth.merchant.id, params.id, merchantContext)
    await auditFromRequest(auth, request, { action: 'food_order.cancel', entityType: 'order', entityId: params.id })
    return result
  }, { params: foodOrderParams })
