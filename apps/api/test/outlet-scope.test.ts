...
describe('food-order outlet isolation (P2)', () => {
  ...
  beforeAll(async () => {
    ...
    scopeB = await loginAs('scope-b@acme.com', 'scope-pass-123456')
  ...
  afterAll(async () => {
    const [merchant] = await db.select().from(users).where(eq(users.email, 'scope-b@acme.com'))
    if (merchant) {
