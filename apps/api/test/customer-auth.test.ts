...
describe('Storefront customer accounts', () => {
  ...
  it('rejects a wrong password', async () => {
    const res = await call('/api/store/jamicore-store/auth/login', json({ email: EMAIL, password: 'wrong-password' }))
  ...
  it('returns the profile for a valid token', async () => {
    const res = await call('/api/store/jamicore-store/auth/me', get('/api/store/jamicore-store/auth/me', token))
    expect(res.status).toBe(200)
  ...
  it('updates the profile name and phone', async () => {
    const res = await call(
      '/api/store/jamicore-store/auth/me',
  ...
  it('lists only this customer\'s orders with line items', async () => {
    const res = await call('/api/store/jamicore-store/auth/orders', get('/api/store/jamicore-store/auth/orders', token))
    expect(res.status).toBe(200)
  ...
  it('rejects missing or invalid tokens on guarded routes', async () => {
    const noToken = await call('/api/store/jamicore-store/auth/me', get('/api/store/jamicore-store/auth/me'))
    expect(noToken.status).toBe(401)

    const badToken = await call(
      '/api/store/jamicore-store/auth/me',
      get('/api/store/jamicore-store/auth/me', 'not.a.jwt')
    )
