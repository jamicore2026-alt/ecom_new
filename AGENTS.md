...
> NOTE: migration `0022_fair_nightcrawler.sql` added `orders.warehouse_id` (nullable FK→warehouses, set null) + `orders_warehouse_idx` — multi-warehouse checkout integration. Storefront `checkout`/`createProviderCheckout` now resolve a fulfilling warehouse (explicit `fulfillmentWarehouseId` from `checkoutBody`/`CheckoutInput`, else the merchant's default **active** warehouse; null → legacy global-inventory path) and, when one resolves, record it on the order **and** decrement that warehouse's `warehouse_inventory` (floor 0, skips variants the warehouse doesn't stock). Global `product_variants.inventory` stays the authoritative sellable quantity. New `test/warehouse-checkout.test.ts` (3 tests). Run `bun run db:migrate`. `bun test`: **174 pass / 0 fail** across 22 files.
> NOTE (2026-08-29): tooling batch — root `eslint.config.js` flat config (ts+svelte; fully clean), `lint` + `typecheck` scripts on all apps, `adapter-auto` removed from web+storefront, logout endpoint now accepts an empty JSON body (was 400). commits `c7e08e2`. Separately added CSV export/import: `GET /api/orders/export`, `GET /api/customers/export` (any staff), `POST /api/customers/import` (multipart file, `CustomersService.importCsv`, upsert by email, fires `customer.import` audit + `customer.created` webhook, gated by new granular **`customers.write`** permission added to `Permission`/`PERMISSIONS`). Follows the existing products CSV pattern via `shared/csv.ts` (`parseCsv`/`toCsv`/`csvEscape` — no new dep). New `test/customers-csv.test.ts` (6 tests). `bun test`: **180 pass / 0 fail** across 23 files.
...
## Phase 8 — Multi-warehouse checkout (2026-08-29)

Integrates the (previously API-only) warehouses module into storefront order fulfillment: each order records the warehouse it was fulfilled from and decrements that warehouse's stock, while `product_variants.inventory` stays the authoritative sellable quantity (drives display/thresholds/cart).

- **Schema** (migration `0022_fair_nightcrawler.sql`, run `bun run db:migrate`): `orders.warehouse_id` nullable FK→warehouses (`onDelete set null`) + `orders_warehouse_idx`.
- **Checkout allocation** (`modules/storefront/service.ts`, in `createOrderTx`): new `fulfillmentWarehouseId?` on `CheckoutInput` + `checkoutBody` (max 30). `resolveFulfillingWarehouse(merchantId, overrideId?)` = explicit active warehouse, else merchant's default **active** warehouse, else `null` (legacy global-inventory path — no warehouse accounting, backward compatible). When a warehouse resolves it is recorded on the order, and every tracked-inventory line calls `allocateWarehouseStock(tx, …)` — a `FOR UPDATE` decrement of `warehouse_inventory` floored at 0 that **skips variants the warehouse doesn't stock** (no row → no-op, never blocks the sale since global stock is authoritative).
- **Verification**: `bun run typecheck` (api) PASS · `bun test`: **174 pass / 0 fail** across 22 files (new `test/warehouse-checkout.test.ts`, 3 tests). No storefront/web changes (merchant-side concept; storefront just omits `fulfillmentWarehouseId` and the API picks the default).
- **Design notes**: global inventory is unchanged everywhere; warehouses are a per-location fulfillment ledger layered on top. Test gotcha: warehouse API writes (`POST /warehouses` `PUT /warehouses/:id/inventory`) live behind `requirePermission('inventory:write')` and are JSON-body routes — tests must send `content-type: application/json` or Elysia 400s `VALIDATION_ERROR: Expected object` (auth header alone isn't enough).
...
- Roadmap Phase 2 (next): CSV export/import (products/customers/orders ✅ 2026-08-29); Phase 3: fulfillment tracking (API ✅), abandoned-cart queue (persistence + conversion wired 2026-08-29; not pg-boss), outbound merchant webhooks ✅, multi-warehouse (API ✅, **checkout-integrated 2026-08-29**), i18n ar+RTL (missing)
